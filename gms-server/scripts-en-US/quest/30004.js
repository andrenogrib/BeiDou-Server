...
function start(mode, type, selection)
{
	if (CheckStatus(mode))
	{
	    ...
	    {
			//第一层对话
			qm.sendAcceptDecline("Dear adventurer, has your journey gone smoothly? Have you decided to leave this place?");
	    }
		else if (status == 1 )
		{
			//第二层对话
			qm.sendOk("Alright, if you've truly decided to leave, please talk to me again.");
			qm.forceStartQuest();
            qm.dispose();	
		}
		else
...
function end(mode, type, selection)
{
	if (CheckStatus(mode))
	{
	    ...
	    {
			//第一层对话
            qm.sendOk("Alright, I'll see you out right away. Have a safe trip!");
