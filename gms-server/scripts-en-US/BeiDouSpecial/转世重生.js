...
function action(mode, type, selection) {
    ...
    if (status === 0) {
        cm.sendNext("When you want to be reborn again, come find me. You currently have a total of #r" + cm.getChar().getReborns() + " #krebirths.");
    } else if (status === 1) {
        ...
        if (selection === 0) {
            if (cm.getChar().getLevel() === cm.getChar().getMaxClassLevel()) {
                ...
                cm.sendOk("It looks like your adventure isn't over yet... Come back when you reach level " + cm.getChar().getMaxClassLevel() +".");
                cm.dispose();
            }
        } else if (selection === 1) {
        ...
        cm.sendYesNo("Are you sure you want to be reborn as an Explorer?");
    }
    else if (status === 4 && type === 1) {
        cm.getChar().executeRebornAsId(jobId);
        cm.dropMessage(0,"Congratulations " + cm.getPlayer() + " on completing rebirth number " + cm.getChar().getReborns() + "");
        cm.dispose();
    }
}