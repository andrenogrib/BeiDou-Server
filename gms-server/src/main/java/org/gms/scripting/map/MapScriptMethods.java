...
public class MapScriptMethods extends AbstractPlayerInteraction {
    ...
    private final String rewardstring = " medal challenge is complete! Please find the Medal Elder to claim your medal.";
    ...
    public void explorerQuest(short questid, String questName) {
        ...
        if (status.equals(infoex)) {
            etm.append("Obtained the ").append(questName).append(" medal!");
            smp.append("You obtained the <").append(questName).append(">").append(rewardstring);
            getPlayer().sendPacket(PacketCreator.getShowQuestCompletion(quest.getId()));
        } else {
            getPlayer().sendPacket(PacketCreator.earnTitleMessage(status + "/" + infoex + " areas explored"));
            etm.append("Challenging the ").append(questName).append(" medal");
            smp.append("You are challenging the ").append(questName).append(" medal. ").append(status).append("/").append(infoex);
        }
    ...
    public void touchTheSky() { //29004
        ...
        getPlayer().sendPacket(PacketCreator.earnTitleMessage("Standing at the Summit medal challenge is in progress"));
        if (Integer.toString(qs.getMedalProgress()).equals(qs.getInfoEx(0))) {
            showInfoText("Standing at the Summit" + rewardstring);
            getPlayer().sendPacket(PacketCreator.getShowQuestCompletion(quest.getId()));
        } else {
            showInfoText("Standing at the Summit medal challenge is in progress. " + status + "/5 completed");
        }
    }
}
